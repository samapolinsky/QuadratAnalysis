...
3. Download dependencies. Packages installed during initial implementation include:
   - Backend: `opencv-python`, `fastapi`, `python-multipart`, `openai`, `uvicorn`, `os`, `csv`
   - Frontend: `React-data-grid`
4. Set your API key in `openai_context.py`
5. Open two local terminals. In one, navigate to `/backend` and run `uvicorn main:app --reload`. In the other, navigate to `/quadrat_analysis` and run `npm run dev`.
6. On the browser, open 'http://localhost:5173/'